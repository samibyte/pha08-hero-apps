import { IoStar } from "react-icons/io5";
import { PiDownloadSimpleBold } from "react-icons/pi";

const AppCard = ({ appData }) => {
  const { title, image, downloads, ratingAvg } = appData;

  return (
    <div
      className="card w-full rounded-sm gap-4
    shadow-lg cursor-pointer  bg-white hover:bg-base-300 hover:-translate-y-2 transition-all duration-200 p-4"
    >
      <figure className=" h-88 overflow-hidden ">
        <img
          className="w-full object-cover rounded-lg"
          src={image}
          alt="app logo"
        />
      </figure>
      <div className="card-body gap-4">
        <h2 className="card-title">{title}</h2>
        <div className="flex justify-between items-center font-medium">
          <div className="flex gap-2 items-center text-[#00D390] py-1 px-2 bg-[#F1F5E8] rounded-sm">
            <PiDownloadSimpleBold />
            <p>{downloads}</p>
          </div>
          <div className="flex gap-2 items-center text-[#FF8811] bg-[#FFF0E1] py-1 px-2 rounded-sm">
            <IoStar />
            <p>{ratingAvg}</p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AppCard;
